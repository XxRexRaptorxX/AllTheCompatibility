...
import xxrexraptorxx.allthecompatibility.main.ATC;
import xxrexraptorxx.allthecompatibility.main.References;
...
public class Events {
        ...
        if (Config.UPDATE_CHECKER.get()) {
            if (!hasShownUp && Minecraft.getInstance().screen == null) {
                if (VersionChecker.getResult(ModList.get().getModContainerById(References.MODID).get().getModInfo()).status() == VersionChecker.Status.OUTDATED ||
                    ...
                    ATC.LOGGER.error(References.NAME + "'s version checker failed!");
                    hasShownUp = true;
    ...
    private static boolean SupporterCheck(URL url, Player player) {
        try {
            ...
            ATC.LOGGER.error("Supporter list URL not found! >>" + url);
